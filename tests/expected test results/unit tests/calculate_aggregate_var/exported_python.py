...
# Constants:
constants_X = [1, 2, 3, 4, 5]

# All tasks:

# Task X_var:
if isinstance(constants_X, (dict, pd.DataFrame)):
    tasks_X_var = pd.DataFrame([{k: np.var(v, axis=0) for k, v in constants_X.items()}])
else:
    tasks_X_var = np.var(constants_X, axis=0)

# All Outputs:

# Output report:
header = True
outputs_report = tasks_X_var
