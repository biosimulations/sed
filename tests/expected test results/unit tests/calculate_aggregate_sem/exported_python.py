...
# Constants:
constants_X = [1, 2, 3.5, 4, 5]

# All tasks:

# Task X_sem:
if isinstance(constants_X, (dict, pd.DataFrame)):
    tasks_X_sem = pd.DataFrame([{k: sem(v, axis=0) for k, v in constants_X.items()}])
else:
    tasks_X_sem = sem(constants_X, axis=0)

# All Outputs:

# Output report:
header = True
outputs_report = tasks_X_sem
