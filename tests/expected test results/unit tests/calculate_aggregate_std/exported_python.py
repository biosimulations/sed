...
# Constants:
constants_X = [1, 2, 3, 4.4, 5.55]

# All tasks:

# Task X_std:
if isinstance(constants_X, (dict, pd.DataFrame)):
    tasks_X_std = pd.DataFrame([{k: np.std(v, axis=0) for k, v in constants_X.items()}])
else:
    tasks_X_std = np.std(constants_X, axis=0)

# All Outputs:

# Output report:
header = True
outputs_report = tasks_X_std
