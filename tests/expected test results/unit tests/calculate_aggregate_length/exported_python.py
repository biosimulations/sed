...
# Constants:
constants_X = [1, 2, 0, 4, 5, 0.0]

# All tasks:

# Task X_length:
if isinstance(constants_X, (dict, pd.DataFrame)):
    tasks_X_length = pd.DataFrame([{k: np.size(v, axis=0) for k, v in constants_X.items()}])
else:
    tasks_X_length = np.size(constants_X, axis=0)

# All Outputs:

# Output report:
header = True
outputs_report = tasks_X_length
