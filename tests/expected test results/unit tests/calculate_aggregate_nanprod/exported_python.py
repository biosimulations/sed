...
# Constants:
constants_X = [1, 2, float('nan'), 4, 0.5]

# All tasks:

# Task X_nanprod:
if isinstance(constants_X, (dict, pd.DataFrame)):
    tasks_X_nanprod = pd.DataFrame([{k: np.nanprod(v, axis=0) for k, v in constants_X.items()}])
else:
    tasks_X_nanprod = np.nanprod(constants_X, axis=0)

# All Outputs:

# Output report:
header = True
outputs_report = tasks_X_nanprod
