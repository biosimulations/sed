...
# Constants:
constants_X = [0, 1, float('nan'), 5, -10, 20, 50]

# All tasks:

# Task X_nanmean:
if isinstance(constants_X, (dict, pd.DataFrame)):
    tasks_X_nanmean = pd.DataFrame([{k: np.nanmean(v, axis=0) for k, v in constants_X.items()}])
else:
    tasks_X_nanmean = np.nanmean(constants_X, axis=0)

# All Outputs:

# Output report:
header = True
outputs_report = tasks_X_nanmean
